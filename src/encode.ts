...
export default function encode(dataset: RDF.BaseQuad[]): Buffer {
	...
	const headerTerms: RDF.Term[] = []
	const headerIndices: Map<string, number> = new Map([["", 0]])
...
const match = (quad: Q, pivots: Q) =>
	pivots.every((pivot, i) => isNaN(pivot) || quad[i] === pivot)
...
function findTree(
	...
	const frequency: Map<number, Q> = new Map()
	let maxCount = min
	let maxTerm = NaN
	let maxPosition = NaN
...
function* pack(
	...
	if (depth > 1) {
		for (
			let tree = findTree(quads, pivots, threshhold);
			tree !== null;
			tree = findTree(quads, pivots, threshhold)
		) {
...
function* packQuads(quads: Matrix) {
	const pivots: Q = [NaN, NaN, NaN, NaN]
	yield* varint.encode(quads.length)
	yield* pack(quads, pivots, 4)
}

function populateURIs(term: RDF.Term, terms: Map<string, RDF.Term>) {
	if (term.termType === "NamedNode") {
		...
		if (blankNodePattern.test(term.value)) {
			...
			throw new Error(
				`Invalid blank node label ${term.value}. The dataset must be canonized first.`
			)
		}
	}
}
