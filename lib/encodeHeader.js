...
export default function encodeHeader(terms) {
    ...
    for (const term of terms) {
        if (term.termType === "NamedNode") {
            header.push(Uint8Array.from(varint.encode(term.value.length * 2 + 1)), encoder.encode(term.value));
        }
            ...
            if (term.language !== "") {
                header.push(Uint8Array.from(varint.encode(term.language.length * 2 + 1)), encoder.encode(term.language));
            }
            else if (term.datatype.value !== xsdString) {
                const index = namedNodeIndices.get(term.datatype.value);
                if (index === undefined) {
                    throw new Error(`Could not find named node ${term.datatype.value} in header`);
                }
